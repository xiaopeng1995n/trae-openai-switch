...
func (p *ProxyServer) Start(ctx context.Context) error {
	...
	reverseProxy.Director = func(req *http.Request) {
		...
		// 处理路径前缀替换
		// 如果目标 URL 包含路径（如 /v2），我们将原始请求的 /v1 替换为该路径
		originalPath := req.URL.Path
		if targetURL.Path != "" && targetURL.Path != "/" {
			targetPath := strings.TrimSuffix(targetURL.Path, "/")
			if strings.HasPrefix(req.URL.Path, "/v1/") {
				req.URL.Path = targetPath + req.URL.Path[3:]
			} else if req.URL.Path == "/v1" {
				req.URL.Path = targetPath
			}
		}

		log.Printf("[Proxy] %s %s -> %s://%s%s", req.Method, originalPath, req.URL.Scheme, req.URL.Host, req.URL.Path)
	}
